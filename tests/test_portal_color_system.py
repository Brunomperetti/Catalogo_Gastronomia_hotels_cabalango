...
def test_public_templates_invalidate_cached_portal_styles():
    ...
    for template_path in Path("app/templates").glob("*.html"):
        ...
        if "path='css/portal.css'" in template:
            expected_version = "?v=20260917-home-hero-composition-1" if template_path.name == "descubri_cabalango.html" else "?v=20260821-travel-guide-3" if template_path.name == "como_llegar.html" else "?v=20260904-provider-single-gallery-1" if template_path.name == "prestador.html" else "?v=20260901-agenda-card-alignment-2" if template_path.name in {"actividades.html", "agenda.html"} else "?v=20260903-event-detail-flyer-contain-1" if template_path.name == "actividad_detalle.html" else "?v=20260914-accommodation-cards-1" if template_path.name == "portal_prestadores.html" else default_version
